...
def main():
    ...
    try:
        ...
        try: orchestrator_proc.wait(timeout=5)
        except subprocess.TimeoutExpired: orchestrator_proc.kill()
        log.info("✅ 協調器已關閉。")
...
def create_runner_script(project_path: Path):
    """將內嵌的 runner 腳本內容寫入檔案。"""
    runner_dir = project_path / "runner"
    runner_dir.mkdir(exist_ok=True)
    runner_script_path = runner_dir / "main_runner.py"
    runner_script_path.write_text(RUNNER_SCRIPT_CONTENT, encoding='utf-8')
    ...
    try:
        ...
        onclick_js = f'''(async () => {{
            try {{
                ...
                this.innerText="✅ 已複製!";
            }} catch (err) {{ this.innerText="❌ 複製失敗"; }}
             finally {{ setTimeout(() => {{ this.innerText="📋 複製這 {num_logs} 條日誌"; }}, 2000); }}
        }})()'''.replace("\n", " ")
        ...
        <details style="margin-top: 15px; margin-bottom: 15px; border: 1px solid #e0e0e0; padding: 12px; border-radius: 8px; background-color: #f9f9f9;">
            ...
            <div style="margin-top: 12px;">
                {top_button_html}
                <pre id="{unique_log_id}" style="background-color: #fff; padding: 12px; border: 1px solid #e0e0e0; border-radius: 5px; white-space: pre-wrap; word-wrap: break-word; font-family: monospace; font-size: 13px; color: #444;"><code>{escaped_log_content}</code></pre>
                {bottom_button_html}
            </div>
        </details>'''
    except Exception as e:
...
def launch_application(project_path_str: str):
    ...
    db_path = project_path / "launcher_logs.db"
    ...
    runner_proc = None
    try:
        log_manager.log("INFO", "正在生成啟動器腳本...")
        runner_script_path = create_runner_script(project_path)
        log_manager.log("SUCCESS", f"✅ 啟動器腳本已生成於: {runner_script_path}")

        command = [sys.executable, str(runner_script_path)]
        runner_proc = subprocess.Popen(
            command, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, encoding='utf-8', bufsize=1
        )
