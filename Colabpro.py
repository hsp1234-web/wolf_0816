...
def download_repository():
    ...
    print(f"  - 倉庫 (Repository): {REPOSITORY_URL}")
    print(f"  - 分支 (Branch/Tag): {TARGET_BRANCH_OR_TAG}")
    print("="*60)
...
def launch_application(project_path_str: str):
    ...
    try:
        ...
        # 核心改動：不再內嵌腳本，而是期望它存在於 Git 倉庫中
        if not runner_script_path.exists():
            raise FileNotFoundError(f"找不到啟動器腳本: {runner_script_path}。請確保此檔案已存在於您下載的 Git 分支中。")
