...
def install_dependencies():
    ...
    requirements_path = ROOT_DIR / "requirements-server.txt"
    if not requirements_path.exists():
        log.error(f"❌ 找不到依賴檔案: {requirements_path}")
        return False
    ...
    try:
        ...
        # 步驟 1b: 使用 uv 安裝依賴
        # 在容器化/Colab 環境中，我們直接安裝到系統環境中
        # 這與原始 Colabpro.py 的行為一致
        command = [
