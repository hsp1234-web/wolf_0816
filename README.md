...
```
服務啟動後，腳本會顯示 API 伺erv器的監聽位址。您可以按下 `Ctrl+C` 來隨時關閉所有服務。

此啟動器內建兩層保護機制：
*   **10 秒日誌看門狗**：如果服務在 10 秒內沒有任何活動，將會自動關閉。
*   **100 秒全局超時**：作為最終保障，腳本運行 100 秒後會自動退出。

更多技術細節，請參閱 **[`DOC/AGENTS.md`](./DOC/AGENTS.md)**。
