...
本專案的背景任務（如 YouTube 下載、音訊轉錄、AI 報告生成）由一系列獨立的、自我管理的「工作者」腳本負責。每個工作者都會自動建立並管理自己的 Python 虛擬環境，以確保依賴的隔離與一致性。

**適用場景**：
*   當您只需要執行特定的背景任務，而不想啟動完整的 Web 服務時。
*   針對特定的工作者進行開發或除錯。

**如何使用**:

每個工作者都支援 `--test` 參數，這會將一個範例任務加入佇列並執行，方便快速驗證。

```bash
# 啟動 YouTube 下載工作者 (會持續監聽任務)
python run_youtube_worker.py

# 啟動模擬的音訊轉錄工作者
python run_transcription_worker.py

# 啟動模擬的 AI 報告生成工作者
python run_ai_report_worker.py
```
