...
@app.get("/api/models/local", response_class=JSONResponse)
async def get_local_models():
    """
    檢查所有已知的 Whisper 模型，並回傳已在本機下載的列表。
    """
    available_models = [
...
async def download_model_task(request: Request):
    """
    接收模型大小，如果模型不存在，則建立一個下載任務。
    """
    payload = await request.json()
    model_size = payload.get("model_size")
    if not model_size or model_size not in KNOWN_WHISPER_MODELS:
        raise HTTPException(status_code=400, detail="無效或未提供的 'model_size'。")

    if check_model_exists(model_size):
        return JSONResponse(
            status_code=200,
            content={"status": "exists", "message": f"模型 '{model_size}' 已存在。"}
        )

    download_task_id = str(uuid.uuid4())
    log.info(f"建立手動模型下載任務: {model_size} (任務 ID: {download_task_id})")

    download_payload = {"model_size": model_size}
    db_client.add_task(download_task_id, json.dumps(download_payload), task_type='download')

    return {"task_id": download_task_id, "type": "download", "message": "模型下載任務已建立。"}
