## 2025-08-26T08:37:16+08:00

### feat(api,frontend): 新增功能狀態API並整合至前端

- **動機**: 根據使用者提供的「後端與前端協同作業改善計畫書」，解決前端 UI 與後端功能實際狀態不同步的問題。
- **後端變更**:
    - 在 `services/api_gateway/main.py` 中新增了 `GET /api/v1/status` 端點，作為回報功能啟用狀態的單一事實來源。
    - 在 `schemas.py` 和 `config.py` 中新增了相應的 Pydantic 模型與設定。
- **前端變更**:
    - 建立了新的 Pinia store `stores/system.js` 來統一管理功能狀態。
    - 修改了 `App.vue`，使其在應用啟動時呼叫新的 API 端點以獲取狀態。
    - 修改了 `YouTubeReporter.vue`，使其 UI 元件根據從後端獲取的狀態動態禁用，並提供禁用原因的提示。
- **測試**:
    - 增強了 `e2e_test.py`，加入新的測試案例以驗證 `/api/v1/status` 端點的回應。
    - 確保了完整的 E2E 測試套件在所有變更後依然能成功通過。
...
- **核心變更**:
    1.  **建立性能測試腳本 (`performance_test.py`)**:
        ...
        - **自動驗證**: 透過輪詢健康檢查端點 (`/`) 來判斷服務是否可用，并包含超時機制。
...
- **增強服務啟動時的穩健性 (Improved Service Startup Robustness)**:
    ...
    - **解決方案**: 以一個可靠的 TCP 健康檢查迴圈取代了固定延遲，確保 API 伺ervidor 完全就緒後，才啟動依賴它的背景服務。
