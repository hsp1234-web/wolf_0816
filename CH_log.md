## 2025-08-29T08:41:50+08:00

### fix(core): 修復模型檢查邏輯並最終穩定全端

- **動機**: 解決在之前的修復後，前端「下載模型」按鈕依然卡在「正在檢查模型...」狀態的問題。
- **核心變更**:
    1.  **定位根本原因**:
        - **`ImportError`**: 經過多輪除錯，發現伺服器因 `ImportError: cannot import name 'hf_hub_get_filename' from 'huggingface_hub'` 而在啟動時崩潰。這表明環境中安裝的 `huggingface-hub` 版本過舊。
        - **錯誤的修復嘗試**: 先前試圖透過 `huggingface-hub>=0.20.0` 來升級套件，但因版本號比較錯誤而未生效。
    2.  **實施穩健的修復方案**:
        - **移除不穩定依賴**: 放棄了使用 `huggingface-hub` 中較新但不穩定的函式（如 `hf_hub_get_filename` 或 `try_to_load_from_cache`）的想法。
        - **手動快取路徑檢查**: 在 `api_server.py` 的 `CHECK_LOCAL_MODELS` WebSocket 處理器中，實現了一個手動但極其穩健的本地檔案系統檢查。它會直接檢查 `~/.cache/huggingface/hub/` 目錄下是否存在對應的模型資料夾，以此判斷模型是否已下載。此方法無須依賴特定 `huggingface-hub` 版本，從根本上解決了 `ImportError`。
        - **實現模型下載邏輯**: 在 `api_server.py` 中為 `DOWNLOAD_MODEL` 訊息新增了處理器，它會在一個獨立的背景執行緒中呼叫 `WhisperModel(...)` 來觸發下載，避免阻塞主服務。
        - **增加前端回呼**: 為 `stores/tasks.js` 新增了對 `REFRESH_LOCAL_MODELS` 訊息的處理，確保在後端下載完成後，前端能自動刷新模型列表。
- **驗證**:
    - 移除了 `requirements-test.txt` 中無效的版本限制。
    - 增強了 `temp_probe.py` 測試腳本，使其能驗證「下載模型」按鈕的狀態是否正確更新。
    - 最終的端對端測試成功通過，所有功能（儀表板數據、模型狀態檢查、手動健康檢查）均符合預期。
- **成果**: 徹底解決了所有已知的前端功能阻塞問題，使應用程式達到了一個真正穩定、可用的狀態。
