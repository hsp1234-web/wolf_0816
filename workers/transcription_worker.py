...
def process_transcription(file_path: str, original_filename: str):
    ...
    add_log.delay(worker_name, "INFO", f"--- 開始處理任務: {original_filename} ---")
    add_log.delay(worker_name, "DEBUG", f"  - 檔案路徑: {file_path}")

    # 模擬耗時的 AI 處理
    add_log.delay(worker_name, "INFO", "正在進行模擬轉錄...")
    time.sleep(10) # 模擬 10 秒的處理時間

    result = f"這是 '{original_filename}' 的模擬轉錄結果。"
    add_log.delay(worker_name, "SUCCESS", f"  - 轉錄完成: {result}")
    add_log.delay(worker_name, "INFO", f"--- 任務完成: {original_filename} ---")
    return result
