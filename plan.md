...
- **標準化服務保護機制**:
    ...
    - **全局超時 (Global Timeout)**: 為避免任何意外的死鎖或無效等待，所有長時間運行的腳本或服務啟動流程都應設定全局超時。
        - **標準值**: **100 秒**。
        - **行為**: 當總執行時間超過此上限時，應立即中止程序並回報錯誤。
    - **日誌看門狗 (Log Watchdog)**: 對於需要長時間運行的服務，必須實施日誌看門狗來監控其健康狀況。
        - **標準值**: **20 秒**。
        - **行為**: 如果在指定時間內沒有收到任何新的日誌輸出，代表服務可能已卡住或「假死」，應立即中止程序並回報錯誤。
