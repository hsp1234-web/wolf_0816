...
### 3.1. 例外：`Colabpro.py` 的完整模式

**重要：** 位於專案根目錄的 `Colabpro.py` **不受**上述模擬規則的限制。

- **用途**：此腳本是專為在 Google Colab 環境中進行「完整功能驗證」而設計的啟動器。
- **行為**：執行 `Colabpro.py` 時，它**必須**下載並安裝所有真實的依賴，包括 `requirements-worker.txt` 中列出的重量級套件，並以**真實模式**（而非模擬模式）啟動所有服務。
- **目標**：確保開發者有一個能夠直接操作、驗證完整功能的線上環境。
