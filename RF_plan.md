...
根據 `performance_test.py` 腳本的首次執行結果，紀錄核心性能指標如下：

- **烘烤耗時 (Baking Time)**   : 1.50 秒
- **啟動耗時 (Launch Time)**    : 2.14 秒
- **首次啟動總耗時 (Total)**  : 3.64 秒
...
| 方案 | 狀態 | 說明 |
| :--- | :---: | :--- |
| **方案零：建立穩定的依賴管理基礎設施** | 🟢 **已完成** | 已成功實作「預先烘烤」策略，解決了依賴安裝的根本問題。 |
| **方案一：統一後端架構** | 🟢 **已完成** | 已建立 `src/main.py` 作為統一入口，並將背景任務整合為 Huey 執行緒。 |
| **方案二：建立模型註冊表** | 🟢 **已完成** | 已建立 `models.json` 並提供對應的 API 端點。 |
| **方案三：實現「分段啟動」** | ❌ **已取消** | 經測量，「預先烘烤」策略的解壓縮速度極快 (0.36秒)，使此方案不再必要。 |
...
經過對 `CH_log.md` 的詳細回顧和對系統行為的觀察，我們將問題根源歸納為以下兩大類：
...
我們建議採取「**預先烘烤依賴 + 統一後端 + 模型註冊表**」的綜合策略，分階段解決上述所有問題。

### 方案零：建立穩定的依賴管理基礎設施 (Pre-baking Strategy)

**核心思想**: 不在運行時動態安裝依賴，而是在開發/建置階段，將所有依賴預先安裝到一個目錄中，並將其打包成單一檔案進行分發。在應用程式啟動時，只需解壓縮並將其路徑加入 `sys.path` 即可。

**具體流程**:
1.  **烘烤階段 (線下完成)**:
    *   執行 `scripts/bake_dependencies.sh` 腳本。
    *   此腳本使用 `uv` 將 `requirements-unified.txt` 中的所有依賴安裝到一個臨時的 `_build/deps` 目錄。
    *   然後將該目錄打包成一個 `dependencies.tar.gz` 壓縮檔。
2.  **執行階段 (沙箱內執行)**:
    *   主啟動器 `run.py` 在啟動初期，執行以下步驟：
        *   將 `dependencies.tar.gz` 解壓縮到一個安全的系統暫存位置（例如 `/tmp/baked_deps_...`）。
        *   執行 `sys.path.insert(0, ...)`，將此路徑添加到 Python 解譯器的搜尋路徑的最前端。
    *   之後，所有 `import` 語句都會優先從我們「烘烤」好的依賴中載入模組。

**預期成果**:
*   **完全規避環境限制**: 整個過程只涉及一次解壓縮操作，檔案數量少，且不會在受監控的目錄下建立新資料夾。
*   **極高的穩定性**: 移除了所有網路請求、即時編譯等不確定因素，確保每次啟動的環境都完全一致。
*   **啟動速度極快**: 經實測，解壓縮耗時僅約 0.36 秒，省去了冗長的安裝時間，應用程式可以幾乎瞬時啟動。
...
**背景**: 在嘗試實施「方案三：分段啟動」的過程中，我們遭遇了由沙箱環境限制引發的嚴重穩定性問題。直接在啟動時使用 `pip install` 安裝大型 AI 依賴的方案，被證明是不可行的。本附錄旨在記錄我們遇到的挑戰、探索的解決方案，以及最終確定的、更穩健的依賴管理策略。

### A. 遭遇的環境限制

經過多次失敗的測試，我們確認此沙箱環境存在以下兩個核心限制：
1.  **禁止建立新目錄**: 正如 `AGENTS.md` 所述，任何在 `/app` 目錄下建立新資料夾的嘗試，都會觸發檔案系統監控的 Bug，導致沙箱永久鎖死。
2.  **檔案操作數量限制 (推測)**: 任何單一指令如果觸發了大量的檔案 I/O 操作（例如 `pip install` 時建立上百個小檔案），都有極高的機率導致環境超時或狀態不一致。`pip` 的安裝過程恰恰是此類操作的典型。

### B. 「分段啟動」方案的挑戰

最初的方案三，建議在背景執行 `pip install -r requirements-ai.txt`。這個方案在理論上可行，但在實踐中失敗了，原因如下：
*   **安裝過程超時**: 安裝 `torch` 等大型套件，在受限的沙箱環境中耗時遠超單一指令的執行時間上限（約 6-7 分鐘），導致測試和執行必定失敗。
*   **測試的複雜性**: 為了驗證這個非同步安裝流程，我們需要撰寫複雜的非同步測試，處理多個事件迴圈的協調問題。這在不穩定的環境中被證明是極其困難且脆弱的。

### C. 最終策略：「預先烘烤」依賴 (Pre-baking Strategy)

為了解決上述根本問題，我們決定採納一種更可靠、更具確定性的策略，即「預先烘烤」所有 Python 依賴。

**核心思想**: 不在運行時動態安裝依賴，而是在開發/建置階段，將所有依賴預先安裝到一個目錄中，並將其打包成單一檔案進行分發。在應用程式啟動時，只需解壓縮並將其路徑加入 `sys.path` 即可。

**具體流程**:
1.  **烘烤階段 (線下完成)**:
    *   在一個乾淨的環境中，執行 `pip install -r requirements.txt --target=./dependencies`，將所有依賴（包括核心和 AI 的）安裝到一個名為 `dependencies` 的本地資料夾。
    *   將整個 `dependencies` 資料夾打包成一個壓縮檔，例如 `dependencies.zip`。
    *   將 `dependencies.zip` 提交到版本庫中。

2.  **部署階段 (沙箱內執行)**:
    *   在 `src/main.py` 啟動的初期，執行以下步驟：
        *   將 `dependencies.zip` 解壓縮到一個安全的位置（例如 `/tmp/deps`）。
        *   執行 `sys.path.insert(0, '/tmp/deps')`，將此路徑添加到 Python 解譯器的搜尋路徑的最前端。
    *   之後，所有 `import` 語句都會優先從我們「烘烤」好的依賴中載入模組。

**優點**:
*   **完全規避環境限制**: 整個過程只涉及一次解壓縮操作，檔案數量少，且不會在受監控的目錄下建立新資料夾。
*   **極高的穩定性**: 移除了所有網路請求、即時編譯等不確定因素，確保每次啟動的環境都完全一致。
*   **啟動速度極快**: 省去了冗長的安裝時間，應用程式可以幾乎瞬時啟動。

這個策略將作為我們未來管理依賴的標準作業流程。

---

## Colab 啟動體驗優化

**目標**: 基於「預先烘烤依賴」的成功，全面優化 Colab 環境的啟動流程，解決舊有設計的體驗問題。

### 遇到的問題

1.  **啟動延遲**: 舊方案（如 `Colabpro.py.legacy`）雖然能啟動通道，但使用者需要等待所有通道嘗試完畢，才能看到最終的網址列表，感知上的等待時間很長。
2.  **UI 閃爍**: 舊方案若採用高頻率刷新，可能會因為元件複雜度而導致 Colab 輸出區塊的閃爍，影響觀感。
3.  **Localtunnel 密碼**: 舊版的 Localtunnel 實作需要使用者手動處理一個隨機產生的密碼，這增加了操作步驟，影響了啟動流程的順暢性。

### 採用的解決方案

為了提供一個快速、流暢且直觀的純文字啟動介面，我們採用了以下組合方案：

1.  **並行通道建立**:
    *   **實作**: 使用 Python 的 `threading` 模組，將 Cloudflare、Localtunnel 和 Colab 官方代理三種通道的啟動邏輯，分別放在獨立的執行緒中並行處理。
    *   **效果**: 使用者無需等待最慢的通道，任何一個通道成功建立，其網址就會立刻出現在畫面上，大幅縮短了「可用性」的感知等待時間。

2.  **Localtunnel 免密碼**:
    *   **實作**: 在啟動 Localtunnel 的 `subprocess` 指令中，加入了 `--bypass-tunnel-reminder` 指令行參數。
    *   **效果**: 徹底繞過了 Localtunnel 的警告頁面和密碼要求，實現了點擊即用的無縫體驗。

3.  **漸進式純文字刷新 UI**:
    *   **實作**: 在 `run.py` 的主執行緒中，建立一個高頻率的 `while` 迴圈。該迴圈會持續讀取由各通道執行緒更新的共享狀態字典。
    *   在每次迭代中，迴圈會使用 `output.clear()` 清除儲存格輸出，然後根據共享字典的當前狀態（例如，哪些網址已獲取，哪些還在「正在產生...」），重新 `print` 整個純文字 UI。
    *   **效果**: 實現了無閃爍的動態更新。使用者可以即時看到每個通道的獲取進度，提供了清晰、直觀的啟動過程反饋。
