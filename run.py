...
def prepare_dependencies(deps_archive_path_str):
    """
    檢查並解壓縮指定的 `dependencies.tar.gz`，然後切換工作目錄並回傳解壓後的路徑。
    """
...
def main():
    """主執行函數，現在只負責啟動 Uvicorn 伺服器。"""
    parser = argparse.ArgumentParser(description="核心 Web 伺服器啟動器")
    parser.add_argument("--deps-path", required=True, help="預先烘烤的依賴壓縮檔路徑。")
    ...
    try:
        ...
        if not port:
            print("啟動器錯誤：找不到可用的埠號。", file=sys.stderr)
            sys.exit(1)

        # 將埠號設定為環境變數，以便其他潛在的腳本可以存取
        ...
        print(f"啟動器錯誤：啟動應用程式時發生未預期的錯誤: {e}", file=sys.stderr)
        sys.exit(1)
