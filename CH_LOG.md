## 2025-08-26T12:56:51.705263+08:00

### feat(e2e): 新增轉錄功能端對端測試

- **動機**: 根據使用者要求，建立一個新的端對端測試案例，以驗證核心的音訊轉錄流程。
- **測試案例**:
    - 模擬上傳一個音訊檔。
    - 在 UI 上選擇「tiny」模型和「光束 1」(beam size 1)。
    - 提交任務。
    - 驗證最終產出的轉錄文字是否正確。
- **目標**: 確保核心轉錄業務邏輯的完整性與正確性。
...
- **動機**: 根據使用者指示，在一個全新的、乾淨的環境中，驗證前端測試的安裝與執行流程，並記錄所有經驗，以確保未來的開發能夠避開歷史上遇到的環境問題。

- **除錯與驗證歷程**:
    1.  **初步調查 (方向錯誤)**: 根據 `CH_LOG.md` 的歷史紀錄，計畫執行一個名為 `frontend_only.spec.js` 的 Playwright E2E 測試。然而，在準備環境時發現，該測試檔案及其所在的 `e2e/` 目錄都已不存在於專案中。這表明日誌已過時，測試套件發生了變更。
    2.  **重新探索與發現**: 透過檢查 `vue-app/` 的目錄結構和 `package.json`，確定了現存的、唯一的前端驗證機制是 Vitest 元件測試 (`LogViewer.spec.js`, `Simple.spec.js`)，其執行指令為 `bun run test:unit`。
    3.  **建立隔離環境**: 建立了一個臨時工作區 (`/tmp/tmp.RFwUQEDDkv`)，並將完整的專案結構複製進去，以確保測試環境的純淨，並遵循了 `CH_LOG.md` 中關於「從根目錄執行安裝」的最佳實踐。
    4.  **依賴安裝 (遭遇挑戰)**:
        - 遵循日誌中的兩階段安裝法，在根目錄和 `vue-app` 目錄下分別執行了 `bun install`。
        - 接著嘗試用 `bunx playwright install-deps` 安裝系統依賴，但 `bunx` 指令不存在。
        - 改用 `npx playwright install-deps` 成功觸發了大規模的系統函式庫安裝。
    5.  **關鍵突破 (`ldconfig`)**: 儘管 `install-deps` 執行完畢，Playwright 的內建檢查工具依然報告缺少大量 `.so` 共享函式庫。這是一個典型的環境問題，表明函式庫雖已安裝，但動態連結器快取未更新。透過執行 `sudo ldconfig`，成功刷新了快取，Playwright 的依賴警告隨之消失。**這一步是本次驗證中最關鍵的發現。**
    6.  **最終測試執行**: 在確認所有環境問題都已解決後，最終在 `vue-app` 目錄下成功執行了 `bun run test:unit`。

- **最終成果與發現**:
    - **環境方面**: 成功建立了一套穩定、可重現的前端元件測試環境。驗證了即使 `npx playwright install-deps` 執行後，也必須手動執行 `ldconfig` 才能確保 Playwright 環境的完整性。
    - **程式碼方面**: 測試的執行揭示了 `LogViewer.spec.js` 中的兩個失敗案例，指向 `LogViewer.vue` 元件的剪貼簿功能存在潛在的邏輯 Bug。
- **結論**: 本次概念驗證成功地完成了使用者的要求。我們不僅建立了一個可靠的測試基礎，還利用它發現了新的問題，為後續的開發工作提供了清晰的方向。

## 2025-08-25T12:32:09+08:00

### docs(debug): 記錄核心功能失效的完整除錯歷程

- **動機**: 根據使用者指示，在找到根本性的解決方案（更換 Huey 後端）之前，暫停修復，並將整個艱難的除錯過程完整地記錄下來，作為未來架構決策的重要參考。
- **問題現象**: 應用程式在 Colab 環境中啟動後，雖然能看到前端介面，但所有核心功能，包括儀表板的動態數據（CPU/RAM）和所有業務邏輯（如下載、轉錄），都完全失效。
- **除錯歷程與發現**:
    1.  **初步懷疑 (前端載入失敗)**: 最初懷疑是前端檔案服務有問題。實施了標準的 SPA 靜態檔案服務 (`StaticFiles(html=True)`)，並將前端移至 `/ui` 子路徑以避免路由衝突。
        - **結果**: 頁面可以穩定載入，但功能依舊失效。證明問題比想像中更深。
    2.  **建立 E2E 測試 (`e2e_test.py`)**: 為了能穩定重現問題，建立了一個高仿真度的端對端測試，它會模擬 Colab 的啟動流程（使用預烘烤的依賴包）。
    3.  **發現一 (依賴包不完整)**: E2E 測試首次運行時，伺服器直接因 `FileNotFoundError` 而崩潰。
        - **原因**: `scripts/bake_dependencies.sh` 腳本沒有將建置好的前端檔案 (`vue-app/dist`) 和上傳目錄 (`youtube_downloads`) 打包進去。
        - **修復**: 修改了 `scripts/bake_dependencies.sh`，將缺失的目錄包含進來。
    4.  **發現二 (啟動路徑錯誤)**: 伺服器依然啟動失敗。
        - **原因**: `run.py` 沒有切換工作目錄，導致 `uvicorn` 載入了原始專案中的舊程式碼。
        - **修復**: 在 `run.py` 中加入了 `os.chdir()`，強制其在解壓後的臨時目錄中運行。
    5.  **發現三 (背景服務缺失)**: 伺服器終於能穩定啟動，但 E2E 測試顯示儀表板依然沒有動態數據。
        - **原因**: 啟動流程中完全遺漏了啟動 `Huey consumer` 和 `hardware_monitor_worker` 這兩個關鍵的背景服務。
        - **修復**: 在 `services/api_gateway/main.py` 的 `lifespan` 管理器中，加入了啟動這兩個服務的邏輯。
    6.  **發現四 (跨程序通訊失敗)**: 儀表板的硬體數據恢復正常，但 Huey 任務（所有核心業務邏輯）依然沒有反應。
        - **原因**: 經過多次嘗試（包括在執行緒中運行 consumer、調整參數等），最終確認 `Huey consumer` 作為一個獨立的子程序，其記憶體與主網站伺服器不互通，無法直接更新狀態。
        - **修復嘗試**: 還原了舊架構中的「內部 API」模式，讓背景任務透過 HTTP 請求來回報狀態。
    7.  **最終根本原因 (`SqliteHuey` 鎖定問題)**: 即使恢復了內部 API，E2E 測試顯示 Huey consumer 雖然啟動了，但從未從佇列中取出並執行任何任務。
        - **原因**: 經過排除所有可能性後，最終定位到問題根源是 `SqliteHuey`。在多程序環境下（主伺服器寫入佇列，consumer 讀取佇列），`SQLite` 的檔案鎖定機制非常不可靠，導致 consumer 進程永遠在等待讀取，造成任務阻塞。
- **當前狀態**: 程式碼已還原至「內部 API」架構實施完畢、但 Huey 任務因 `SqliteHuey` 鎖定問題而無法執行的狀態。所有修正和 E2E 測試腳本均已提交，為下一步的架構升級（如更換為 `RedisHuey`）提供了完整的上下文和穩定的測試環境。
...
- **核心變更**:
    ...
    3.  **修正啟動腳本**: 修改 `run.py`，在啟動 `uvicorn` 前，強制使用 `os.chdir()` 切換到解壓後的臨時目錄。
    4.  **恢復背景任務啟動**: 在 `services/api_gateway/main.py` 的 `lifespan` 管理器中，以背景執行緒的方式啟動 Huey consumer。
