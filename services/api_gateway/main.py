...
async def lifespan(app: FastAPI):
    ...
    huey_thread = threading.Thread(target=run_huey_consumer, daemon=True, name="HueyConsumer")
    threads.append(huey_thread)
    huey_thread.start()
...
async def system_update(payload: Dict):
    """接收來自背景工作者的系統狀態更新，並透過 WebSocket 廣播。"""
    # 直接將收到的 payload 廣播出去，因為 hardware_monitor_worker 已經組裝好了完整的訊息
