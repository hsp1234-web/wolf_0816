    ...
    async with async_playwright() as p:
        try:
            ...
            await page.wait_for_function(
                "() => document.querySelector('#cpu-label')?.textContent.trim() !== '--'",
                timeout=15000
            )
            print("✅ [Probe] CPU/RAM 數據已成功載入。")

            print("👀 [Probe] 正在等待模型檢查完成 (下載按鈕變為可用)...")
            await page.wait_for_function(
                "() => document.querySelector(`button[data-testid='download-model-button']`)?.textContent.includes('下載模型')",
                timeout=20000 # 檢查模型可能需要一些時間
            )
            print("✅ [Probe] 模型狀態檢查成功，「下載模型」按鈕已啟用。")
