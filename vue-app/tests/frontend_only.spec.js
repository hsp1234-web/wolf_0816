...
test('前端互動與指令斷言測試', async ({ page }) => {
  ...
  expect(fileTask.type).toBe('transcribe');
  expect(fileTask.source).toBe('file');
  ...
  expect(youtubeTask.type).toBe('report');
  expect(youtubeTask.source).toBe('youtube');
  expect(youtubeTask.payload.video_url).toBe('https://www.youtube.com/watch?v=dQw4w9WgXcQ');
